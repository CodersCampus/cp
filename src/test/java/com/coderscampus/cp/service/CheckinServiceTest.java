...
public class CheckinServiceTest {
	...
	@AfterEach
	void cleanUpData() {
		// go through student checkinDTOLists and delete each one, one at a time
		...
		student2CheckinDTOList.stream().forEach(checkinDTO -> {
			Checkin checkin = checkinRepo.findById(checkinDTO.getId()).get();
			checkinRepo.delete(checkin);
		});
		// then delete both students
...
//
		...
		// since using checkinService.findByUid in prev tests, index is not starting at
		// 0
		// could use streams or arrraylist to have better looking code but it gets job
		// done
		String ids = "";
